...
function ServiceReserveButton({ service }) {
  ...
  return (
    <button
      ...
      disabled={adding}
      className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-medium transition-all duration-200 hover:shadow-md active:scale-95 disabled:opacity-60"
      aria-label={`Reservar ${service.name}`}
    >
      {adding ? 'Agregando...' : 'Reservar'}
    </button>
  );
}
